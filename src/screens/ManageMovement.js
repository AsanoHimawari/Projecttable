...
const ManageMovement = ({ route, navigation }) => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      title: isEditting ? "Update Table" : "Add Table",
    });
  ...
  const confirmHandler = async (tableData) => {
    if (isEditting) {
      await updateMovement(movId, tableData);
      movementsCtx.updateMovement(movId, tableData);
      // update
    } else {
      const id = await storeMovement(tableData);
      movementsCtx.addMovement({ ...tableData, id: id });
    }
    navigation.goBack();
  };
